...
def main(snapshotdate=None, download_data=False):
    """
    ...
    """
    #download weather data using download_weather_data.py
    if download_data:
        print("Downloading weather historical data")
        download_noaa_isd_data(2023,2024) #download data from 2023 to 2024 (2 years of historical data for model train/validation/test)
    ...
    try:
        ...
        # -------------------- WEATHER BRONZE --------------------
        print("\n" + "-"*80)
        print("Processing NOAA ISD weather → Bronze")
        print("-"*80)

        weather_raw_root   = "data/weather_history"
        weather_output_root = "datamart/bronze/weather_history"

        if snapshotdate:
            # OOT mode → write one day, partitioned by snapshot_date
            oot_dates = [snapshotdate]                   # single-day list
            process_bronze_weather_history(
                spark=spark,
                raw_root_dir=weather_raw_root,
                output_root=weather_output_root,
                oot_date_list=oot_dates
            )
            print(f"✓ Weather OOT Bronze written for {snapshotdate} under {weather_output_root}/")
        else:
            # Historical mode → write 2023-01-01 .. 2024-12-31 as a single Parquet file
            # Call without an OOT date list so the function performs the historical write.
            process_bronze_weather_history(
                spark=spark,
                raw_root_dir=weather_raw_root,
                output_root=weather_output_root,
            ...
            print(f"✓ Weather historical Bronze written under {weather_output_root}/")
        # ----------------------------------------------------------------------------- 
...
Examples:
  ...
  python bronze_feature_store.py
  
  # Process single day for OOT inference
  python bronze_feature_store.py --snapshotdate 2025-01-01
  
  # Process another day
  python bronze_feature_store.py --snapshotdate 2025-01-02
        """
    )
